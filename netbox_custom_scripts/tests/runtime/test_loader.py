...
from netbox_custom_scripts.runtime.exceptions import EntrypointImportError, InvalidModulePathError
from netbox_custom_scripts.runtime.naming import PRIVATE_ROOT, project_module_name, revision_module_name
...
from netbox_custom_scripts.tests.storage.test_store import STORAGE_KEY, OpenRecordingStorage, manifest_for
...
ROOT_INIT = b'from netbox_custom_scripts.tests.runtime.test_loader import ROOT_RUNS\nROOT_RUNS.append("run")\n'
...
class FailureBoundaryTestCase(LoaderTestCase):
    ...
    def test_passthrough_exceptions_escape_unwrapped_after_the_sweep(self):
        files = {
            'probe.py': (
                b'from netbox_custom_scripts.tests.runtime.test_loader import FakeTimeout\nraise FakeTimeout()\n'
            ),
        }
        manifest, digest = seed_revision(self.storage, STORAGE_KEY, files)
