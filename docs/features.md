...
| Script File | One declared file that discovery imports and publishes Custom Scripts from. | [Script File](models/scriptfile.md) |
...
| Repair published scripts | Operator | Republish a serving Project's Custom Scripts from the revision it already serves, for rows that drifted from the snapshot they derive from. The Project reports how many rows moved, or that none did. |
| List a Project's files | Operator | Read the current revision's files with size, checksum, and script file state on the Files tab. A declared path the served revision does not hold is annotated, and the annotation says which case it is: gone from the source, or waiting on a newer revision to be activated. |
