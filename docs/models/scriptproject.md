...
| Moving `activation_policy`, `data_source` or `data_path` needs the `activate` permission as well as `change` | The serializer's `validate()`, the edit and bulk-import forms' `clean()`, and the bulk-edit view. Submitting a stored value is not a move, and a create is not gated |
