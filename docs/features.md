...
| List a Project's files | Operator | Read the current revision's files with size, checksum, and entrypoint state on the Files tab. A declared path the served revision does not hold is annotated, and the annotation says which case it is: gone from the source, or waiting on a newer revision to be activated. |
