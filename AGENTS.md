...
│   │   ├── views/test_files.py    , The Files tab: manifest rows, the script file marker, both missing-path annotations (gone from the source versus held only by a newer revision), the empty state, and the view gate.
