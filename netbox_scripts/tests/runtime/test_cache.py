...
from netbox_scripts.runtime import cache
from netbox_scripts.runtime.exceptions import LocalCacheCorruptError, LocalCacheError
from netbox_scripts.storage import store
...
class CacheRootTestCase(TestCase):
    ...
    def test_the_default_root_sits_under_the_system_temporary_directory(self):
        self.assertEqual(
            ...
            Path(tempfile.gettempdir()) / 'netbox-scripts' / 'runtime-cache',
        )

    @override_settings(PLUGINS_CONFIG={'netbox_scripts': {'runtime_cache_root': '/var/cache/custom'}})
    ...
    def test_every_level_this_tier_creates_is_private(self):
        ...
        root = base / 'netbox-scripts' / 'runtime-cache'
...
class S3MaterializationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        boto3.client('s3', region_name='us-east-1').create_bucket(Bucket='netbox-scripts-cache-test')
        self.storage = S3Storage(
            bucket_name='netbox-scripts-cache-test',
