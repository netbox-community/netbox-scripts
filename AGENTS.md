...
│   ├── urls.py                    , urlpatterns for 'modules/' + 'projects/' + 'scripts/' + detail-only 'revisions/<int:pk>/' via get_model_urls, sorted. Segments never repeat the base_url. The three 'migration/' routes are explicit paths, because the passes act on the built-in feature and have no model.
...
│   │   ├── migration.py            , The Migration page plus the two enqueue views, sharing one permission gate. Both passes take add_customscriptproject, because that is what staging produces. Staging confirms first and refuses while a pass is queued, the inventory does neither, because it writes nothing.
...
│       ├── migration.html         , The Migration landing page, which extends `generic/_base.html` rather than an object template.
