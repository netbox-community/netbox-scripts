...
| `netbox.api.viewsets.NetBoxModelViewSet.perform_update` and `perform_destroy` | `api/views.py` | The write hooks the lock-order mixin wraps. It relies on core taking its conditional row lock inside them rather than around them, so a project lock taken outside still lands first. A rename fails here, a move of that row lock out of the hook does not |
...
| `utilities.exceptions.AbortRequest` | `models/projects.py`, `forms/model_forms/projects.py` | Refuses a write from inside a form save or a model save, where core answers 400 over REST and puts the message on the form in the browser. Core documents it for plugins in `docs/plugins/development/exceptions.md` and catches it in `NetBoxModelViewSet.dispatch`, `ObjectEditView.post` and both bulk views |
