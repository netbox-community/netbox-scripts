...
│   │       ├── revisions.py    , ScriptProjectRevisionSerializer: read-only, and also the serializer event serialization resolves by model name. Omits the manifest, the script file snapshot and the lease owner and start fields, and carries last_validation_failure, which is why a revision states on its own page why no verdict was reached.
...
│   │   ├── model_forms/projects.py   , [ScriptProject] ScriptProjectEditForm + ScriptProjectScriptFilesForm (reconciles the selection onto enabled, then enqueues ProjectScriptFileRefreshJob when it moved, and starts a project holding exactly one candidate with it selected, guarded on no declaration existing rather than none being enabled, so an existing disabled declaration is not re-selected. Deselecting the sole candidate before anything is declared writes nothing, so that default is offered again). Its selection is core's SplitMultiSelectWidget, built per instance because a MultiWidget forwards no choices to its two panes, and the choices are optgroups keyed on each path's directory with the source root under `(root)`, so an option carries only the file name and the annotations stay readable in a narrow pane.
...
│   │   └── migration.py           , MigrationRun(ChangeLoggedModel): one attempt at moving off the built-in feature. Migration infrastructure rather than a domain model, so no REST, GraphQL or list view. State moves forward one step only, at most one run is open, and the journal is what every cutover step replays from. complete_step() and recorded_counts() are the one home of the resume guard every caller needs. record_journal(), record_mapping(), record_step() and record_warnings() are the only writers of the two JSON columns. Each reads the row fresh under migration_lock() and applies only the entries the caller passed, never the instance's own copy of the column, so a stale instance cannot drop a nested map another object recorded. Anything a caller assigns onto .journal directly is not persisted, record_journal() is the route.
│   ├── tables/
...
│   │   ├── plugin_testing.py      , [shared] Plugin-aware view/API test mixins (always rendered). PrimaryObjectViewTestCase + NestedObjectViewTestCase + DerivedObjectViewTestCase, the second for a model created and removed through its parent, so read and edit only, the third for models whose rows are derived, so no create, delete, or import. NestedObjectAPIViewTestCase is the API analogue of the second, for a viewset that refuses POST and DELETE by method. Also the one home of ChangeLoggedFilterSetTestMixin, which the three filterset suites import from here rather than from the host.
...
│   │   ├── api/test_scripts.py     , NetBoxScriptSerializer route reversal, event serialization, patchable enabled, ignored derived fields, refused create/delete. + ScriptFileAPIViewTestCase(PluginAPIViewTestCases.NestedObjectAPIViewTestCase): read-only discovery fields, the frozen project and source path, and the 405 on POST to the list route and DELETE to the detail route. Path canonicalization and the three path refusals moved to the model layer, the create route they went through being gone.
...
│   │   ├── views/test_scripts.py   , NetBoxScriptViewSetTestCase(PluginTestCases.DerivedObjectViewTestCase) + detail view, changelog rendering, and the absent create/delete routes. + ScriptFileTestCase(PluginTestCases.NestedObjectViewTestCase), which also pins that the add, delete and bulk delete routes do not reverse and that the detail page offers Edit alone.
...
│   │   ├── views/test_actions.py  , Static guard: every list, detail and row action is checked against the registered routes, because ActionsMixin and ActionsColumn filter by permission alone. Its three inventories are DERIVED from what the views and tables packages export, never listed, because the hand-written list named two of the fourteen the package exports, five of the omitted twelve already offering buttons for routes that do not exist, so it could not see the drift it exists to catch.
│   │   ├── views/test_files.py    , The Revision Files tab: manifest rows, the script file marker, both missing-path annotations (gone from the source versus held only by a newer revision), the line naming the revision it lists and its absence without one, the empty state, and the view gate.
│   │   ├── views/test_migration.py , The Migration page and its seven enqueue routes: both permission gates, every enqueue, the queued-pass refusals, which button each state renders, and the next step the page names, one case per position plus the three states where it names nothing.
...
│   │   ├── forms/test_script_files.py , Selection reconciles onto enabled, nested paths, missing declared paths, and the optgroups: the `(root)` header, an option carrying only its file name, and a nested directory grouping separately from its parent. Plus the lone-candidate default: it is offered rather than declared, it stops at two candidates, and an existing disabled declaration is not re-selected.
...
│   │   ├── migration.py            , The Migration page, the run's detail view, and the seven enqueue views, on TWO gates. BaseMigrationView takes add_scriptproject for the page, the inventory, staging and verification, because creating Projects is all those authorize. DestructiveMigrationView takes migrate_scriptproject for the cutover, activation, repoint and cleanup, because closing and deleting rows of the built-in feature is not a form of creating a Project. The cutover is offered while the crossing is unrecorded rather than while the state reads staging, so the one button that finishes an interrupted crossing stays up, and a notice beside it says the fence may already have closed. Staging, the cutover and cleanup confirm first and refuse while a pass of their own class is queued, the inventory and verification do neither, because they write nothing. The repoint and cleanup buttons are both withheld, with the Projects named, while any of them serves nothing, gated on the frozen map rather than on the recorded step because that is what the predicate reads. Cleanup is withheld for a second reason: it skips a module whose Project serves nothing, so the run could not close and the click would buy nothing. `next_action` names the first pass that is offerable and not yet complete, pairing each button's own render condition with `step_done()` or, for the three passes that record no step, their own Job status. Two of the refusals need a clause of their own: a blocking finding withholds staging, and a Project that stopped serving reopens activation rather than letting the hint run on to the cleanup, which would delete nothing. Verification completes only for a run after the cleanup, since it is offered at every state.
...
│   │   ├── scripts.py               , List/Detail/Edit/BulkEdit views plus Run (GET builds the class's own form out of the active revision, POST enqueues) and Result (one run's log, read out of the Job). No add, delete, bulk delete or bulk import: rows are derived from an activated revision, and retirement replaces deletion. The Jobs tab needs no view, JobsMixin registers one. Run declares a ViewTab gated on the run permission, so every view of the script offers it, and builds its form through execution.script_class_context(), which the REST run action shares. Result serves its body as a partial to an htmx poll, so a run that has not reached a terminal state refreshes itself, at a slower rate while it is only scheduled. + List/Detail/Edit views. No create, delete, bulk delete, bulk edit or bulk import: a declaration is made on the Project, and the three surfaces that would inherit an action for a missing route, the list view, the detail view and the table, each declare it away.
│   │   └── revisions.py             , Detail view for one revision, carrying the script file paths its snapshot froze, which is where the Revisions tab's script file count resolves. That table always renders, unlike the problems one, because an empty snapshot is why a revision publishes nothing. Neither it nor the shared confirmation base offers the clone, edit and delete actions ObjectView defaults to, since a revision registers none of those routes. Plus Activate + Deactivate, GET confirms and POST performs, and activation_message(), the one wording both Activate routes report through. Gated on the PROJECT's activate permission, with the revision queryset narrowed to permitted projects. The tab links here rather than posting: its table is inside the bulk-action form, so a nested form would submit the outer one.
│   ├── ui/
...
│   │   ├── locks.py               , project_lock(): the session-level per-project advisory lock for content operations, taken through django_pg_utils.advisory_lock. project_write_lock(): the same key at transaction scope for database-only project and declaration changes. This module owns the key derivation and the (770100, .) namespace, with models/migration.py deriving (770101, 1) from it. The separation from core is numeric and not by arity, since core takes two-integer pairs too, so docs/development/netbox-internals.md records both values against ADVISORY_LOCK_KEYS.
...
│   │   ├── source.py              , The ONLY module that reads the built-in Custom Scripts feature, so a supported export service is a change to one file. Read-only, returns frozen dataclasses and plain counts. Filters to the SCRIPTS root: the proxy manager admits reports as well, and a report's bytes live under REPORTS_ROOT while every read here goes through the scripts backend, so one could never be read. Reports are outside this migration entirely and legacy_report_count() is what makes that visible. The Job and Event Rule readers SUBTRACT what a report owns rather than selecting the scripts root, because a reference whose object type and id disagree survives an upgrade and the passes report it, and the subtraction is paired per object type since the two key sequences are independent. The source subscription and the permission rows name shared object types and cannot be partitioned. A report action excludes the whole rule, so one that also watches the built-in feature keeps that subscription and no later pass repoints it. Opens `file_path` rather than `full_path`, and resolves both content types with for_concrete_model=False because a Job and an Event Rule record the proxy.
...
│   ├── execution.py               , run_script(): the transaction, request-processor and event context one run happens inside. The only home of the six undocumented NetBox symbols execution needs, so the requested generic core context replaces one file. Also script_class_context(): a stored row to a live class through the revision its project serves, with its import namespace retained until the caller finishes constructing the script and using its form. Model-aware, which is why it is here and not in the runtime tier.
│   ├── permissions.py             , validate_script_file_permissions(): the object-level recheck for the rows a parent write creates or flips, using core's count-against-a-restricted-queryset idiom inside the caller's transaction.
...
│       ├── migration.html         , The Migration landing page, which extends `generic/_base.html` rather than an object template. Lists the last inventory's blocking findings above the buttons, because staging refuses on any of them and creates nothing, and counts the warnings instead, because that list is one entry per module. The seven buttons render in the order the passes run, each numbered, with Verify last even though it sits outside the destructive gate, and the one `next_action` names renders filled while the rest are outlined.
│       ├── migrationrun.html      , Detail-view template for one migration attempt.
│       ├── migration_stage.html   , Confirmation for staging.
│       ├── migration_cutover.html , Confirmation for the fence. Says there is no way back, in those words, and names the reports it does not cover plus the two closures that reach them anyway.
...
Database-only changes to projects and declarations hold `project_write_lock()` on
that same key until the outer transaction ends. This keeps a form's declaration
writes serialized with the next storage snapshot without wrapping backend I/O in
the form transaction. Declaration deletion holds it through `pre_delete` too.
