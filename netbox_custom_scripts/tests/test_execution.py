    ...
    return NetBoxFakeRequest(
        {
            ...
            'user': user,
            'method': 'POST',
            'path': '/plugins/custom-scripts/scripts/1/run/',
            'id': uuid.uuid4(),
        }
    )
...
class RequestProcessorTestCase(ExecutionTestMixin, TestCase):
    ...
    def run_with_processors(self, entered, *, commit):
        ...
        with (
            patch('netbox_custom_scripts.execution.event_tracking', new=tracking),
    ...
    def test_a_processor_that_cannot_start_does_not_end_the_run(self):
        ...
        with (
            ...
            self.assertLogs('netbox.plugins.netbox_custom_scripts.execution', 'WARNING') as logged,
        ):
            run_script(instance, data={}, commit=True)
    ...
    def test_event_tracking_failing_to_start_does_end_a_committed_run(self):
        ...
        with (
            patch('netbox_custom_scripts.execution.event_tracking', new=broken_tracking),
