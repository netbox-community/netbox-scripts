...
class StorageBackendTestCase(TestCase):
    ...
    @override_settings(STORAGES=storages_config(netbox_custom_scripts=IN_MEMORY))
    ...
    def test_the_backend_is_resolved_on_each_call_rather_than_captured(self):
        ...
        with override_settings(STORAGES=storages_config(netbox_custom_scripts=IN_MEMORY)):
            self.assertIsInstance(config.get_storage(), InMemoryStorage)
        with override_settings(STORAGES=storages_config(netbox_custom_scripts=file_system)):
            self.assertIsInstance(config.get_storage(), FileSystemStorage)

    @override_settings(STORAGES=storages_config(netbox_custom_scripts={'BACKEND': 'nowhere.NoSuchStorage'}))
...
class StorageCheckTestCase(TestCase):
    ...
    def test_a_missing_entry_is_reported_as_w001(self):
        ...
        self.assertEqual([message.id for message in messages], ['netbox_custom_scripts.W001'])
