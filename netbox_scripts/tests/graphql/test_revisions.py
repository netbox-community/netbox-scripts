...
from netbox_scripts.graphql.types import ScriptProjectRevisionType


class ScriptProjectRevisionGraphQLTestCase(SimpleTestCase):
    ...
    def test_status_exposed_as_a_raw_value(self):
        ...
        field = next(
            item for item in ScriptProjectRevisionType.__strawberry_definition__.fields if item.name == 'status'
        )
