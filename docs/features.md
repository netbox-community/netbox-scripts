...
| List a Project's files | Operator | Read the current revision's files with size, checksum, and entrypoint state on the Files tab. A declared path missing from the source is annotated. |
